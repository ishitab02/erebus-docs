...
> = {
 ...
 "operation_id": "op_5a11...09dc"}`,
    detail:
      "This is the only example on the page shown from the payee's session, because a client cannot counter its own offer: replying crosses directions, and trying it returns NOT_YOUR_OFFER. So reply_to names the offer the other side made, and the counter that comes back is the caller's own.",
  },
  wait_for_offers: {
 ...
 "result": {
   ...
   "settlements": [], "timed_out": false}}`,
    detail:
      "The count is over every offer the channel holds, not only new ones, so a payer that has already proposed waits for 2 to see one reply. Countering does not withdraw: the replied-to offer moves to countered but stays acceptable.",
  },
  read_channel_state: {
